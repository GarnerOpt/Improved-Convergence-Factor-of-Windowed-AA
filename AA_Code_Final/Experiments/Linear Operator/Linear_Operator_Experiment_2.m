...
min_diff = inf;
%plot([1,180],[rhs,rhs],'k--','LineWidth',1.5);
hold on;
for ii=1:100
    x0 = randn(n,1);
...
%     plot(LHS{1},'r-','LineWidth',1.5);
%     hold on;
%     plot(LHS{2},'b-','LineWidth',1.5);
%     hold on;
%     plot(LHS{3},'g-','LineWidth',1.5);
%     hold on;
end
...
% ylabel('Values in Ratio Bound','FontSize',14,'FontWeight','bold')
% axis([0,245,0, 0.90])
...
legend({'FP','AA(1)','AA(2)','AA(3)','Thm. Est.'},'FontSize',14,'FontWeight','bold','Location','best');
xlabel('Iterations (k)','FontSize',16,'FontWeight','bold');
ylabel('r_{est}(k)','FontSize',16,'FontWeight','bold')
axis([0,245 ,0.75, 1.2])
title('Comparing R-Linear Convergence Factors','FontSize',16,'FontWeight','bold');
